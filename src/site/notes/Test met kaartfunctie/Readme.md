---
{"dg-publish":true,"permalink":"/test-met-kaartfunctie/readme/"}
---
...
For example [[Test met kaartfunctie/Eiffel Tower\|Eiffel Tower]] has the following properties:
...
Instead of getting the icon and color from the note, we can also get them from the note's assigned type. In this example, [[Musée d'Orsay\|Musée d'Orsay]] is assigned the type [[Test met kaartfunctie/Museums\|Museums]]. To get the map marker from the Museum type we use [formula](https://help.obsidian.md/bases/functions) properties called **Type icon** and **Type color**:
...
See the [[Test met kaartfunctie/Museums\|Museums]] note for an example of a map that only displays markers for its assigned type.